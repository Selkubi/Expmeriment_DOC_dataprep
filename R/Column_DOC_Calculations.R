...
ggplot(data_all)+
  facet_grid( ~variable)+
  geom_boxplot(aes(y=(normalized_doc), x=sample_date, fill=sample_date))+
  theme_boxplot()+fill_selected()+
  ylab("Biomass Normalized ΔDOC \n (Δ um C/L*Bacterium?)")

ggplot(data_all)+
  facet_grid( ~variable)+
  geom_boxplot(aes(y=(Cell_pro_ml), x=sample_date, fill=sample_date))+
  theme_boxplot()+fill_selected()+
  ylab("Biomass (cell/ml sample)")
